...
# Default Arch uses xz (slow, ~3-4h). We use zstd -15 for ~60% speedup.
# NOTE: mksquashfs options are version-sensitive:
#   - '-Xbcj x86'    → only valid for xz compression (NOT zstd)
#   - '-T0'          → invalid; mksquashfs uses -processors <N> instead
#                      (and by default it already uses all CPUs)
#   - '-Xcompression-level' → valid for zstd (range: 1..22, default 15)
# So we keep it simple: just -comp zstd + level + block size.
